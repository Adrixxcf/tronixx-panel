...
function renderCuentas() {
  ...
  // Pre-calcular qué cuenta "hereda" las ventas sin cuentaId por plataforma
  // (la primera cuenta creada de cada plataforma recibe las ventas sin vincular)
  const cuentaPorPlataforma = {};
  [...cuentas].sort((a,b) => {
    const ta = a.creadoEn?.seconds || 0;
    const tb = b.creadoEn?.seconds || 0;
    return ta - tb;
  }).forEach(c => {
    if (!cuentaPorPlataforma[c.plataforma]) cuentaPorPlataforma[c.plataforma] = c._id;
  });

  grid.innerHTML = list.map(c => {
    const k = pKey(c.plataforma);
    const esPrincipal = cuentaPorPlataforma[c.plataforma] === c._id;

    const ventasDeCuenta = ventas.filter(v => {
      if (v.cuentaId === c._id) return true;
      // Fallback solo para la cuenta principal de esa plataforma
      if (!v.cuentaId && v.plataforma === c.plataforma && esPrincipal) return true;
      return false;
    });
    ...
    return `
    <div class="cuenta-card${cuentaEstaVencida ? ' cuenta-card-vencida' : ''}">
      ${alertaBanner}
