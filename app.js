...
function renderCuentas() {
  ...
  grid.innerHTML = list.map(c => {
    ...
    const nActivos = ventas.filter(v =>
      v.cuentaId === c._id && v.estado === 'Activo'
    ).length;
    const nVActivos = ventas.filter(v => v.cuentaId === c._id && (v.estado === 'Activo' || v.estado === 'Pendiente' || v.estado === 'Corte pendiente')).length;
...
function getCuentasDisponibles(excluirVentaId = null) {
  // Excluir cuentas vendidas como completa activa/pendiente
