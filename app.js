...
window.saveVenta = async () => {
  ...
  const rec = {
    ...
    fecha,
    fechaVenc:  document.getElementById('fv-fechaVenc').value,
...
window.changeVentasPage = d => { ventaPage=Math.max(0,ventaPage+d); renderVentas(); };

function filteredVentas() {
  const q  = (document.getElementById('search')?.value||'').toLowerCase();
  const pl = document.getElementById('filter-platform')?.value||'';
  const tp = document.getElementById('filter-tipo')?.value||'';
  ...
  return ventas
    .filter(v => {
      ...
      return mQ && mP && mT && mS && mM;
    })
    .sort((a,b) => {
      let va=a[ventaSortCol]||'', vb=b[ventaSortCol]||'';
      if (ventaSortCol==='precio'){va=parseFloat(va)||0;vb=parseFloat(vb)||0;}
      return va<vb?-ventaSortDir:va>vb?ventaSortDir:0;
    });
}

/* ── AUTO-CORTE: marca como "Corte" si venció y estaba Activo/Pendiente ── */
async function autoCorteVencidos() {
  const hoy = new Date(); hoy.setHours(0,0,0,0);
  for (const v of ventas) {
    if (!v.fechaVenc) continue;
    if (v.estado === 'Corte' || v.estado === 'Vencido') continue;
    const venc = new Date(v.fechaVenc + 'T00:00:00');
    if (venc < hoy) {
      try { await updateDoc(doc(db,'ventas',v._id), { estado: 'Corte' }); }
      catch(e) { console.error('Auto-corte error:', e); }
    }
  }
}
...
function renderVentas() {
  const rows  = filteredVentas();
  const total = rows.length;
  if (ventaPage*PS>=total && ventaPage>0) ventaPage=Math.max(0,Math.ceil(total/PS)-1);
  const slice = rows.slice(ventaPage*PS, ventaPage*PS+PS);
  const tbody = document.getElementById('ventas-body');
  document.getElementById('ventas-count').textContent = `${total} registro${total!==1?'s':''}`;

  if (!slice.length) {
    tbody.innerHTML='<tr class="empty-row"><td colspan="10">No hay registros para mostrar.</td></tr>';
  } else {
    tbody.innerHTML = slice.map(v => {
      const cuenta = cuentas.find(c=>c._id===v.cuentaId);
      const k    = pKey(v.plataforma);
      // Estado: Activo=verde, Corte=rojo, Pendiente=amarillo, Vencido=rojo
      ...
      const tipoBadge = v.tipo==='cuenta'
        ? '<span class="tipo-badge tipo-cuenta">★ Completa</span>'
        : '<span class="tipo-badge tipo-perfil">◈ Perfil</span>';
      ...
      // Botón renovar solo si está en Corte o Vencido
      const renovarBtn = (v.estado==='Corte' || v.estado==='Vencido')
        ? `<button class="icon-btn renew" title="Renovar" onclick="renovarVenta('${v._id}')">↻</button>`
        : '';
  ...
  const from = total ? ventaPage*PS+1 : 0;
  const to   = Math.min(ventaPage*PS+PS, total);
  document.getElementById('ventas-page-info').textContent = total ? `${from}–${to} de ${total}` : '0 registros';
  document.getElementById('prev-btn').disabled = ventaPage===0;
  document.getElementById('next-btn').disabled = (ventaPage+1)*PS>=total;
}
...
window.exportCSV = () => {
  const rows=filteredVentas();
  const cols=['N° Contacto','Plataforma','Tipo','Perfil/Cuenta','Correo Cuenta','F.Venta','F.Vencimiento','Precio(S/.)','Estado','Notas'];
  const lines=[cols.join(','), ...rows.map(v=>{
    const c=cuentas.find(x=>x._id===v.cuentaId);
    return [v.numero||'',v.plataforma||'',v.tipo||'',v.perfil||'',c?.correo||'',v.fecha||'',v.fechaVenc||'',v.precio||0,v.estado||'',v.notas||'']
      .map(x=>`"${String(x).replace(/"/g,'""')}"`).join(',');
  })];
  const blob=new Blob(['\uFEFF'+lines.join('\n')],{type:'text/csv;charset=utf-8;'});
  const a=document.createElement('a'); a.href=URL.createObjectURL(blob);
  a.download=`tronixx_ventas_${new Date().toISOString().split('T')[0]}.csv`; a.click();
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Poblar filtros de plataforma (ventas y cuentas) — FIX del filtro que no funcionaba
  ['filter-platform','filter-plat-cuentas'].forEach(selId => {
    const sel = document.getElementById(selId);
    if (!sel) return;
    while (sel.options.length > 1) sel.remove(1); // limpiar opciones fijas del HTML
    PLATFORMS.forEach(p => {
