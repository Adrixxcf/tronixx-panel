...
/* Refrescar select de cuentas en modal de venta */
...
function renderVentas() {
  ...
  if (!slice.length) {
    ...
    tbody.innerHTML = slice.map(v => {
      ...
      const sl   = v.estado==='Activo'?'s-active':v.estado==='Vencido'?'s-expired':'s-pending';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const formSelV = document.getElementById('fv-cuenta-id');
  PLATFORMS.forEach(p=>{
