...
function renderCuentas() {
  ...
  grid.innerHTML = list.map(c => {
    ...
    // Contar ventas vinculadas por cuentaId (ventas nuevas) O por plataforma+correo (ventas antiguas sin cuentaId)
    const ventasDeCuenta = ventas.filter(v => {
      if (v.cuentaId === c._id) return true;
      // Fallback: misma plataforma y sin cuentaId asignado
      if (!v.cuentaId && v.plataforma === c.plataforma) return true;
      return false;
    });
