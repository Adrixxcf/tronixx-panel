...
function renderCuentas() {
  ...
  grid.innerHTML = list.map(c => {
    ...
    const nV = ventas.filter(v => v.cuentaId === c._id).length;
...
window.renovarVenta = async id => {
  ...
  try { await updateDoc(doc(db,'ventas',id), { estado:'Activo', fecha:hoy, fechaVenc:sumarMes(hoy) }); }
...
function renderMetrics() {
  ...
  // Gastos: suma de costos de cuentas (convirtiendo USD a PEN con tasa ~3.7)
  const USD_A_PEN = 3.7;
  const gastos = cuentas.reduce((s,c) => {
    const costo = parseFloat(c.costo) || 0;
    if (costo === 0) return s;
    return s + (c.moneda === 'USD' ? costo * USD_A_PEN : costo);
  }, 0);
  ...
  document.getElementById('metrics').innerHTML = `
    ...
    <div class="metric"><div class="metric-icon">🛒</div><div class="metric-label">Gastos</div><div class="metric-value red">S/. ${gastos.toFixed(2)}</div><div class="metric-sub">costo cuentas</div></div>
