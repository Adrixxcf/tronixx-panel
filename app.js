...
window.renovarVenta = id => {
  ...
  // Precio anterior y campo nuevo precargado
  document.getElementById('renovar-precio-anterior').textContent =
    v.precio ? `S/. ${parseFloat(v.precio).toFixed(2)}` : '—';
  document.getElementById('renovar-precio').value = v.precio || '';
...
window.confirmarRenovacion = async () => {
  ...
  const nuevoPrecio   = parseFloat(document.getElementById('renovar-precio').value) || 0;
